...
__all__ = [
    ...
    'LambdaSampler',
    'synchronize_state',
    'RandomRandomTemporalSegmentSampler',
    'RandomSystematicSampler',
]
...
'''
    Adding for AI CITY 2022
'''

class RandomSystematicSampler(_BaseSampler):
    def __init__(self, n_frames=16,window_size=180):
        super(RandomSystematicSampler, self).__init__(n_frames)
        self.window_size=window_size     
    def _get_sampled_frame_ids(self, source, start_frame, end_frame, sample_id=None):                     
        w_size=self.window_size
        s_frame=start_frame
        e_frame=end_frame
        if  end_frame-start_frame>w_size:            
            # s_frame=np.random.randint(start_frame,end_frame-w_size-1) 
            s_frame=int((end_frame-start_frame)/2)-int(w_size/2)
            e_frame=s_frame+w_size    
        else:
            s_frame=start_frame
            e_frame=end_frame     
            w_size=end_frame-start_frame
        sampled_frame_ids = np.linspace(s_frame, e_frame, self.n_frames)
        return sampled_frame_ids.round().astype(np.int64)
        
class RandomRandomTemporalSegmentSampler(_BaseSampler): # For AI_CITY_2022 window_size 3s
    def __init__(self, n_frames=16,window_size=180):
        super(RandomRandomTemporalSegmentSampler, self).__init__(n_frames)
        self.window_size=window_size
    def _get_sampled_frame_ids(self, source, start_frame, end_frame, sample_id=None):                    
        w_size=self.window_size
        s_frame=start_frame
        ...
        if  end_frame-start_frame>w_size:
            ...
            e_frame=end_frame 
            w_size=end_frame-start_frame
        segments = np.linspace(s_frame, e_frame, self.n_frames + 1)
        segment_length = w_size / self.n_frames
        sampled_frame_ids = segments[:-1] + np.random.rand(self.n_frames) * segment_length
        return sampled_frame_ids.round().astype(np.int64)